...
var PreviewImg = React.createClass({
    render: function () {
        return React.createElement(
            ...
            React.createElement(LoadedText, null),
            React.createElement(
                "div",
                { className: "loadedImgInnr" },
                this.props.data.render.map(function (arrElem) {
                    return React.createElement(
                        "div",
                        { className: "loadedImg" },
                        React.createElement(CloseCross, null),
                        React.createElement(
                            "a",
                            { href: arrElem['src'], download: true },
                            React.createElement("img", { "data-src": arrElem['dataSrc'], src: arrElem['src'], title: arrElem['title'], className: "img-thumbnail pointer" })
                        )
                    );
                })
            )
...
function success(Data, elem, type) {
    ...
    ReactDOM.render(React.createElement(PreviewImg, { data: Data }), document.getElementsByClassName('imgBox')[0]);
                ...
                this.props.data.map(function (arrElem) {
                    ...
                    return React.createElement(
                        ...
                        React.createElement(
                            "p",
                            null,
                            new Date(arrElem['timestamp']).getHours() + ':' + new Date(arrElem['timestamp']).getMinutes() + ':' + new Date(arrElem['timestamp']).getSeconds(),
...
window.msgHandler = new Object({
    ...
    pull: function () {
        ...
        if (Array.isArray(this.msgArr) && this.msgArr.length > 0) {
            ...
            ReactDOM.render(React.createElement(PreviewNf, { data: this.msgArr }), document.getElementById('notify-block'));
    ...
    init: function () {
        ...
        let classStsatusToggle = (elem, currentStatus) => {
            switch (currentStatus) {
                ...
                case false:
                    elem.removeClass('true').addClass('false');break;
                default:
                    return false;
            }
        };
